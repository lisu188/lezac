...
`docs/recovery/runtime_evidence_ledger.md`. The ledger's
`original_runtime_fixture_count` must match the machine-readable `fixture=`
entry count. Each ledger entry and each field in that entry must be unique, keep
