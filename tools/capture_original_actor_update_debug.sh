...
run_environment_preflight() {
    if [[ "${LEZAC_ACTOR_UPDATE_DEBUG_SKIP_ENVIRONMENT_PREFLIGHT:-0}" == "1" ]]; then
        {
    ...
    if [[ $preflight_status -ne 0 ]]; then
        ...
        echo "actor_update_debug_capture=error scenario=$scenario reason=environment_preflight_exit_$preflight_status manifest=$manifest raw_dump=$raw_dump environment_preflight_log=$environment_preflight_log"
...
if [[ "${LEZAC_ACTOR_UPDATE_DEBUG_DRY_RUN:-0}" == "1" ]]; then
    ...
    echo "actor_update_debug_capture=ok mode=dry_run scenario=$scenario route=$runtime_route manifest=$manifest raw_dump=$raw_dump candidate_fixture=$candidate_fixture debugger_commands_runtime=$runtime_commands_file environment_preflight=dry_run environment_preflight_log=$environment_preflight_log"
    exit 0
fi
