...
case "$target" in
    contact_scanner_runtime_sound)
        ghidra=1000:5E81
        label=contact_scanner_cursor_0069
        scenario=contact_scanner_runtime_sound
        cursor=0x0069
        priority=4
...
run_environment_preflight() {
    ...
    if [[ $preflight_status -ne 0 ]]; then
        ...
        echo "sound_callsite_procmem=error target=$target reason=environment_preflight_exit_$preflight_status manifest=$manifest raw_dump=$raw_dump environment_preflight_log=$environment_preflight_log"
...
sample_seconds=${LEZAC_SOUND_CALLSITE_PROCMEM_SAMPLE_SECONDS:-0.5}
sample_interval=${LEZAC_SOUND_CALLSITE_PROCMEM_SAMPLE_INTERVAL:-0.1}
tail_freeze_seconds=${LEZAC_SOUND_CALLSITE_PROCMEM_TAIL_FREEZE_SECONDS:-0.2}
start_taps=${LEZAC_SOUND_CALLSITE_START_TAPS:-2}
level_start_seconds=${LEZAC_SOUND_CALLSITE_LEVEL_START_SECONDS:-3.0}
right_key=${LEZAC_SOUND_CALLSITE_RIGHT_KEY:-x}
right_hold_seconds=${LEZAC_SOUND_CALLSITE_RIGHT_HOLD_SECONDS:-2.0}
bomb_key=${LEZAC_SOUND_CALLSITE_BOMB_KEY:-n}
bomb_hold_seconds=${LEZAC_SOUND_CALLSITE_BOMB_HOLD_SECONDS:-0.25}
route_steps_csv=${LEZAC_SOUND_CALLSITE_ROUTE_STEPS:-}
runtime_freeze_before_route=${LEZAC_SOUND_CALLSITE_RUNTIME_FREEZE_BEFORE_ROUTE:-0}
...
write_candidate_skeleton() {
    ...
    {
        echo "# LEZAC sound-callsite process-memory oracle candidate."
        echo "# Candidate only: fill latch/request rows before promotion."
        ...
        echo "break ghidra=1000:165A runtime=${runtime_cs:-<runtime-cs>}:165A label=sound_priority_latch observed=fill_from_debugger_or_latch_trace"
        echo
        echo "# sound_request label=$label callsite=$ghidra latch=1000:165A cursor=$cursor priority=$priority active_before=<0-or-1> current_priority_before=<n> pending_cursor=$cursor pending_priority=$priority accepted=<0-or-1> active_after=<0-or-1> current_priority_after=<n> current_cursor_after=<cursor> direct_sweep=0"
        ...
        echo
        echo "# dump DS:2070"
        echo "# <runtime-ds>:2070  <bytes around DS:2074 pending cursor>"
        echo "# dump DS:78C0"
        echo "# <runtime-ds>:78C0  <bytes around DS:78c0 current cursor>"
        echo "# dump DS:7990"
        echo "# <runtime-ds>:7990  <bytes around DS:799e current priority and DS:799f pending priority>"
        echo "# dump DS:79C0"
        echo "# <runtime-ds>:79C0  <bytes around DS:79c4 active flag>"
...
{
    echo "capture=sound_callsite_process_memory"
    echo "source=dosbox-debug-process-memory"
    echo "target=$target"
    echo "label=$label"
    echo "scenario=$scenario"
...
{
    echo "sound_callsite_procmem=planned"
    echo "target=$target"
    echo "label=$label"
    echo "scenario=$scenario"
    echo "capture_class=$capture_class"
...
if [[ "${LEZAC_SOUND_CALLSITE_APPROVE_PROCMEM:-0}" != "1" ]]; then
    echo "refusing process-memory capture without LEZAC_SOUND_CALLSITE_APPROVE_PROCMEM=1" >&2
    exit 64
fi
if [[ "${LEZAC_SOUND_CALLSITE_APPROVE_RUNTIME_INSTRUMENTATION:-0}" != "1" ]]; then
    echo "refusing runtime instrumentation without LEZAC_SOUND_CALLSITE_APPROVE_RUNTIME_INSTRUMENTATION=1" >&2
    exit 64
fi
...
{
    echo "sound_callsite_procmem=complete"
    echo "runtime_cs=$runtime_cs"
    echo "runtime_ds=$runtime_ds"
    echo "freeze_runtime=$freeze_runtime"
    echo "freeze_old_bytes=$freeze_old_bytes"
