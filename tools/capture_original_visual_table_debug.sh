...
case "$scenario" in
    ...
    *)
        echo "unsupported visual-table scenario: $scenario" >&2
        exit 65
        ;;
esac
...
run_environment_preflight() {
    if [[ "${LEZAC_VISUAL_TABLE_DEBUG_SKIP_ENVIRONMENT_PREFLIGHT:-0}" == "1" ]]; then
        {
    ...
    if [[ $preflight_status -ne 0 ]]; then
        ...
        echo "visual_table_debug_capture=error scenario=$scenario reason=environment_preflight_exit_$preflight_status manifest=$manifest raw_dump=$raw_dump environment_preflight_log=$environment_preflight_log"
...
log="$out_dir/visual_table_debug_capture.log"
...
BP <CS>:3108  # state-2 death/life-loss helper
BP <CS>:6053  # actor update animation cursor owner
BP <CS>:6148  # actor frame consumer
BP <CS>:7C89  # state-2 countdown
BP <CS>:7DDF  # state-2 return-active path
R
D DS:0060
D DS:79E0
D DS:C21E
...

{
    ...
    echo "environment_preflight_log=$environment_preflight_log"
    echo "anchors=1000:3108,1000:6053,1000:6148,1000:7C89,1000:7DDF"
    echo "visual_claim=0"
} >"$manifest"

cat >"$candidate_fixture" <<EOF_FIXTURE
# LEZAC visual-table runtime oracle candidate
...
#
# break ghidra=1000:3108 runtime=<runtime-cs>:3108 label=state2_death_helper
# break ghidra=1000:6053 runtime=<runtime-cs>:6053 label=actor_update_animation
# break ghidra=1000:6148 runtime=<runtime-cs>:6148 label=actor_frame_consumer
# break ghidra=1000:7C89 runtime=<runtime-cs>:7C89 label=state2_countdown
...
#
# dump DS:C322
# <runtime-ds>:C322  <bytes>
# dump DS:C21E
# <runtime-ds>:C21E  <bytes>
EOF_FIXTURE

{
    echo "visual_table_debug_capture=planned"
    echo "scenario=$scenario"
    echo "route=$runtime_route"
    echo "expected_level=$expected_level"
    echo "anchor_state2_visual_table=1000:3108,1000:6053,1000:6148,1000:7C89,1000:7DDF"
    echo "candidate_fixture=$candidate_fixture"
    echo "debugger_commands_runtime=$runtime_commands_file"
    echo "environment_preflight_command=$(quote_command "${environment_preflight_command[@]}")"
    echo "environment_preflight_log=$environment_preflight_log"
    echo "fixture_command=./build/lezac_cpp --debug-visual-table-oracle <candidate-fixture>"
} >"$raw_dump"
...
if [[ "${LEZAC_VISUAL_TABLE_DEBUG_DRY_RUN:-0}" == "1" ]]; then
    echo "environment_preflight=dry_run" >>"$manifest"
    echo "environment_preflight=dry_run" >>"$raw_dump"
    echo "visual_table_debug_capture=ok mode=dry_run scenario=$scenario route=$runtime_route manifest=$manifest raw_dump=$raw_dump candidate_fixture=$candidate_fixture debugger_commands_runtime=$runtime_commands_file environment_preflight=dry_run environment_preflight_log=$environment_preflight_log"
    exit 0
fi
...
run_dir=$(mktemp -d /tmp/lezac-visual-table-debug.XXXXXX)
cleanup() {
    rm -rf "$run_dir"
}
trap cleanup EXIT
...
dosbox_command=(
    env TERM=xterm-256color xvfb-run -a timeout "${LEZAC_VISUAL_TABLE_DEBUG_TIMEOUT_SECONDS:-20s}"
    dosbox-debug
    -c "mount c $run_dir"
    -c "c:"
    -c "DEBUG LEZAC.EXE"
...
if [[ $status -ne 0 ]]; then
    echo "visual_table_debug_capture=error scenario=$scenario reason=dosbox_debug_exit_$status manifest=$manifest raw_dump=$raw_dump"
    exit "$status"
fi

echo "visual_table_debug_capture=ok mode=capture scenario=$scenario route=$runtime_route manifest=$manifest raw_dump=$raw_dump candidate_fixture=$candidate_fixture"
