#!/usr/bin/env python3
"""Check actor-update DOSBox-debug capture helper wiring."""
...
BREAKPOINTS = ["5CB0", "604F", "6053", "777F"]
...
def check_script(script_path: Path) -> None:
    ...
    require(text, "LEZAC_ACTOR_UPDATE_DEBUG_SKIP_ENVIRONMENT_PREFLIGHT", "script")
    ...
    for dump in ["D DS:7900", "D DS:79E0", "D DS:7A00"]:
...
def check_cmake(cmake_path: Path) -> None:
    ...
    for snippet in [
        ...
        if collapse_ws(snippet) not in collapsed:
            raise RuntimeError(
                "actor_update_debug_capture_helper_expectations missing snippet: "
                f"{snippet}"
            )
...
def check_docs(root: Path) -> None:
    ...
    for text, label in [(readme, "README"), (status, "RECOVERY_STATUS")]:
        require(text, "tools/capture_original_actor_update_debug.sh", label)
        require(text, "object_collision_jump_live", label)
    require(status, "debugger_seeded", "RECOVERY_STATUS")


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check actor-update DOSBox-debug capture helper coverage."
    )
    parser.add_argument(
        "repo_root",
        nargs="?",
    ...
    check_script(root / "tools" / "capture_original_actor_update_debug.sh")
