#!/usr/bin/env python3
"""Check lane-result ready-manifest runner behavior."""
...
def run_ready(
    ...
    command = [
        sys.executable,
        str(root / "tools" / "run_lane_result_ready_manifest.py"),
        *args,
    ]
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check lane-result ready-manifest runner output."
    )
    ...
    cases = 0
    with tempfile.TemporaryDirectory(prefix="lezac-lane-ready-manifest-") as tmp:
        base = Path(tmp)
        fixture = base / "fixture" / "lane_result.txt"
        ...
        for snippet in [
            ...
            "offset_address=1000:3D3F runtime_cs=01ED runtime_ds=0C8F",
        ...
        require(directory_input, "ready_candidates=1", "directory_input")
        cases += 1
        ...
        log_path = log_dir / "candidate_0_3d3f_x2p00_c0p50.log"
        ...
        require(bad, "lane_result_ready_manifest=error", "bad_promotion")
    ...
    print(f"lane_result_ready_manifest_check=ok cases={cases}")
    return 0
