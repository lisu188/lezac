#!/usr/bin/env python3
"""Check visual-claim promotion candidate dry-run validation."""
...
def run_candidate(
    ...
    *extra_args: str,
    ...
    command = [
        sys.executable,
        str(repo_root / "tools" / "validate_visual_claim_promotion_candidate.py"),
        fixture,
        str(bundle),
        str(docs),
        "--repo-root",
    ...
    if expect_success and result.returncode != 0:
        raise RuntimeError(
            f"candidate validator failed with {result.returncode}: {' '.join(command)}\n"
            f"{result.stdout}"
        )
    if not expect_success and result.returncode == 0:
        raise RuntimeError(
            f"candidate validator unexpectedly passed: {' '.join(command)}\n{result.stdout}"
        )
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="lezac-visual-candidate-") as tmp:
        root = Path(tmp)
        fixture = "sample_original.txt"
        write_text(root / "tests" / "fixtures" / "dosbox" / fixture, "visual_claim=0\n")
        docs = root / "docs" / "recovery" / "sample_note.md"
        write_text(docs, "# Sample note\n")
        bundle = make_bundle(root)

        auto = run_candidate(repo_root, root, fixture, bundle, docs)
        for snippet in [
        ...
        require(explicit, "fixture_current_visual_claim=1", "explicit")
        require(explicit, "label=010_level1_start", "explicit")
        cases += 1

        unready_root = root / "unready"
        write_text(unready_root / "tests" / "fixtures" / "dosbox" / fixture, "visual_claim=0\n")
        unready_docs = unready_root / "docs" / "recovery" / "sample_note.md"
        write_text(unready_docs, "# Sample note\n")
        unready_bundle = make_bundle(unready_root, ready=False)
        unready = run_candidate(
            repo_root,
            unready_root,
            fixture,
            unready_bundle,
        ...
        missing_docs = run_candidate(
            repo_root,
            root,
            fixture,
            bundle,
            root / "docs" / "recovery" / "missing.md",
            expect_success=False,
        )
        require(missing_docs, "missing_docs", "missing_docs")
        cases += 1

        bad_fixture = run_candidate(
            repo_root,
            root,
            "../sample_original.txt",
            bundle,
        ...
        require(bad_fixture, "bad_fixture_name=../sample_original.txt", "bad_fixture")
        cases += 1

    print(f"visual_claim_promotion_candidate_check=ok cases={cases}")
    return 0
