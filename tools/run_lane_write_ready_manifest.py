...
EXPECTED_PROMOTION = "lane_write_ready_candidates"
    ...
    for index in range(count):
        ...
        candidates.append(
            ReadyCandidate(
                ...
                lane_write_kind=require(values, f"{prefix}_lane_write_kind"),
                lane_write_target=require(values, f"{prefix}_lane_write_target"),
...
def candidate_result_lines(
    ...
    return [
        ...
        f"{prefix}_lane_write_kind={candidate.lane_write_kind}",
        f"{prefix}_lane_write_target={candidate.lane_write_target}",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run oracle commands from a lane-write ready manifest."
    )
    ...
    if args.allow_missing_fixtures and not args.dry_run:
        print(
            "lane_write_ready_manifest=error "
    ...
    for candidate in candidates:
        ...
        if args.dry_run:
            ...
            print(
                ...
                f"kind={candidate.lane_write_kind} "
                f"target={candidate.lane_write_target} "
    ...
    if args.write_result_manifest is not None:
        try:
            ...
            print(f"lane_write_ready_result_manifest=error reason={exc}", file=sys.stderr)
            return 1
        print(
