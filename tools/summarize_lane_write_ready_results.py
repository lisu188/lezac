...
EXPECTED_RESULT = "lane_write_ready_manifest"
ORACLE_FLAGS = {
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Summarize lane-write ready-manifest oracle results."
    )
    ...
    try:
        ...
        print(f"lane_write_ready_result_summary=error reason={exc}", file=sys.stderr)
        return 1

    counts = status_counts(candidates)
    if failures != counts["error"]:
        print(
            "lane_write_ready_result_summary=error "
