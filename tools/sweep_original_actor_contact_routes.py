...
the lower-level wrapper. Use --all-targets to sweep every mapped actor/contact
process-memory target in one guarded batch.
"""
...
def selected_targets(args: argparse.Namespace) -> list[str]:
    if args.all_targets:
        ...
        return TARGETS
    return args.target or ["contact_scanner_start"]
