#!/usr/bin/env python3
"""Plan or run original behavior-4 process-memory route captures."""
...
SCENARIO_LEVELS = {
    "monster_spawner_behavior4_level2": 2,
    "monster_spawner_behavior4_level3": 3,
    "monster_behavior4_target_selection": 3,
}
...
def selected_targets(args: argparse.Namespace) -> list[str]:
    if args.all_targets:
        ...
        return TARGETS
    return args.target or DEFAULT_TARGETS
...
def env_for_capture(
    ...
    env = {
        "LEZAC_BEHAVIOR4_ROUTE_STEPS": ",".join(route_steps),
        "LEZAC_BEHAVIOR4_PROCMEM_SCENARIO": args.scenario,
        "LEZAC_BEHAVIOR4_PROCMEM_EXPECTED_LEVEL": str(args.expected_level),
        "LEZAC_BEHAVIOR4_PROCMEM_SAMPLE_SECONDS": args.sample_seconds,
        "LEZAC_BEHAVIOR4_PROCMEM_SAMPLE_INTERVAL": args.sample_interval,
        "LEZAC_BEHAVIOR4_PROCMEM_TAIL_FREEZE_SECONDS": args.tail_freeze_seconds,
        "LEZAC_BEHAVIOR4_PROCMEM_SKIP_ENVIRONMENT_PREFLIGHT": "1",
    }
    if timing == TIMING_BEFORE_ROUTE:
        env["LEZAC_BEHAVIOR4_RUNTIME_FREEZE_BEFORE_ROUTE"] = "1"
    if args.dry_run:
        env["LEZAC_BEHAVIOR4_PROCMEM_DRY_RUN"] = "1"
    if args.approve_procmem:
        env["LEZAC_BEHAVIOR4_APPROVE_PROCMEM"] = "1"
    if args.approve_runtime_instrumentation:
        env["LEZAC_BEHAVIOR4_APPROVE_RUNTIME_INSTRUMENTATION"] = "1"
    return env
...
def build_capture_command(
    ...
    command = [
        "bash",
        str(root / "tools" / "capture_original_behavior4_procmem.sh"),
        str(route_out_dir),
        str(args.asset_dir),
        target,
    ]
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Plan or run route-step sweeps for original behavior-4 evidence."
    )
    ...
    parser.add_argument(
        "--scenario",
        choices=sorted(SCENARIO_LEVELS),
        default="monster_behavior4_target_selection",
    )
    ...
    for target in targets:
        for timing in timings:
            for index, route in enumerate(routes):
                label = f"{target}_{timing}_{route_labels[index]}"
    ...
    print(
        "behavior4_procmem_route_sweep=ok "
        f"mode={'dry_run' if args.dry_run else 'capture'} "
        f"out_dir={out_dir} scenario={args.scenario} "
        f"expected_level={args.expected_level} targets={len(targets)} "
    ...
    manifest_lines = [
        ...
        f"expected_level={args.expected_level}",
    ...
    for label, command, env, display, route_out_dir in capture_entries:
        ...
        for line in result.stdout.splitlines():
            if line.startswith("behavior4_procmem=ok"):
                manifest_lines.append(f"capture_status_{label}={line}")
    manifest = out_dir / "manifest.txt"
    manifest.write_text("\n".join(manifest_lines) + "\n", encoding="ascii")
    print(f"behavior4_procmem_route_sweep_manifest={manifest}")
    return 0
